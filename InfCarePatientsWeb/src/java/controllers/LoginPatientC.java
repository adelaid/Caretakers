 ...
 * @author Ada
 */
@WebServlet(name = "LoginPatientC", urlPatterns = {"/LoginPatientC"})
public class LoginPatientC extends HttpServlet {
...
//                request.getSession().setAttribute("LOGIN_ERROR", error);
    ...
    private Patient getPatient(java.lang.String username) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        ws.PatientWS port = service.getPatientWSPort();
        return port.getPatient(username);
    }
